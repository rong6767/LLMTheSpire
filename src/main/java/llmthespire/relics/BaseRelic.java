package llmthespire.relics;

import basemod.abstracts.CustomRelic;
import basemod.helpers.RelicType;
import llmthespire.util.GeneralUtils;
import llmthespire.util.TextureLoader;
...
public abstract class BaseRelic extends CustomRelic {
    ...
    protected void loadTexture() {
        this.img = TextureLoader.getTextureNull(BasicMod.makeRelicPath(imageName + ".png"), true);
        if (img != null) {
            outlineImg = TextureLoader.getTextureNull(BasicMod.makeRelicOutlinePath(imageName + ".png"), true);
            if (outlineImg == null)
                outlineImg = img;
        }
        else {
    ...
    public void loadLargeImg() {
        if (notPng(imageName)) {
            if (largeImg == null) {
                this.largeImg = ImageMaster.loadImage(BasicMod.makeRelicPath("large/" + imageName + ".png"));
            }
        }
        else {
            super.loadLargeImg();
