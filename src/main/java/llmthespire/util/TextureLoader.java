...
import llmthespire.BasicMod;

import java.util.HashMap;
import java.util.Locale;

import static llmthespire.BasicMod.getLogger;
...
public class TextureLoader {
    ...
    public static Texture getTexture(final String filePath, boolean linear) {
        if (textures.get(filePath) == null) {
            try {
                ...
                getLogger().error("Failed to find texture " + filePath, e);
                Texture missing = getTextureNull(BasicMod.makeImagePath("missing.png"), false);
                if (missing == null) {
                    getLogger().info("missing.png is missing, should be at " + BasicMod.makeImagePath("missing.png"));
                }
                return missing;
            }
        }
    ...
    {
        String textureString = BasicMod.makeImagePath("cards/" + cardType.name().toLowerCase(Locale.ROOT) + "/" + cardName + ".png");
        ...
        if (!h.exists())
        {
            switch (cardType) {
                case ATTACK:
                    textureString = BasicMod.makeImagePath("cards/attack/default.png");
                    break;
                case POWER:
                    textureString = BasicMod.makeImagePath("cards/power/default.png");
                    break;
                default:
                    textureString = BasicMod.makeImagePath("cards/skill/default.png");
                    break;
            }
        }
    ...
    private static void loadTexture(final String textureString, boolean linearFilter) throws GdxRuntimeException {
        ...
        getLogger().info("Loaded texture " + textureString);
        textures.put(textureString, texture);
    }

    public static Texture getPowerTexture(final String powerName)
    {
        String textureString = BasicMod.makePowerPath(powerName + ".png");
        return getTexture(textureString);
    }
    public static Texture getHiDefPowerTexture(final String powerName)
    {
        String textureString = BasicMod.makePowerPath("large/" + powerName + ".png");
